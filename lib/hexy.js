...
function Hexy(options) {
  ...
  self.com.on('ready', function () {
    ...
    // Add all moves in `/moves` folder into Hexy.move
    self.moves = {};
    fs.readdir('./moves', function(err, files){
      for(var file in files){
        self.moves[files[file].split(".")[0]] = function(){
          require('../moves/' + files[file].split(".")[0])(self, async);
        };
      }
    });
