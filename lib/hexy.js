...
function Hexy(options) {
  ...
  self.com.on('ready', function () {
    ...
    // Legs params: communication, hipServo, kneeServo, ankleServo, offsetX, offsetY, coeff, [symetry]
    ...
    self.LM = new Leg(self.com, 11, 10, 9, -103.3, 0, 0, true);
...
/**
 * Make the Hexy walk in a direction
 *
 * @param  {Int} degrees       Direction of the move
 * @param  {Int} step          Number of steps
 * @param  {Int} [scaleFactor] Height of steps
 * @api public
 */
 ...
 * @api public
 */
Hexy.prototype.standUp = function () {
  async.series([
    function setHips (next){
      this.RF.hip.position = -30;
      this.RM.hip.position = 1;
      this.RB.hip.position = 30;

      this.LF.hip.position = 30;
      this.LM.hip.position = 1;
      this.LB.hip.position = -30;
      setTimeout(next, 500);
    }.bind(this),

    function setKnees (next){
      for(var leg in this.legs) this.legs[leg].knee.position = -30;
      setTimeout(next, 500);
    }.bind(this),

    function setAnkle (next){
      for(var leg in this.legs) this.legs[leg].ankle.position = -90;
        setTimeout(next,500);
    }.bind(this),
    ...
    function standUp (next){
      async.timesSeries(48/3, function(n, nextTime){
        ...
        for(var leg in this.legs){
          this.legs[leg].knee.position = (n * 3);
          this.legs[leg].ankle.position = -90 + (n * 3);
        }
