 ...
 * @param {Int} offsetY
 * @param {Int} coeff
 * @param {Boolean} [symetry] LM leg case
 *
 **/
function Leg(com, hipServoNum, kneeServoNum, ankleServoNum, offsetX, offsetY, coeff, symetry) {
...
Leg.prototype.setFootY = function (footY) {
  if (footY < 75 && footY > -75) {
    var kneeAngle = Math.degrees(Math.asin(footY / 75));
...
Leg.prototype.replantFoot = function (endHipAngle, stepTime) {
  ...
  async.timesSeries(stepPerS, function (n, next) {
    ...
    footY = footMin - Math.sin(Math.radians(anglNom)) * footRange;
...
Leg.prototype.ikFullLeg = function (x, y, z) {
  // Position offset
  x = this.symetry * (x - this.offsetX);
  y = this.symetry * (y - this.offsetY);
